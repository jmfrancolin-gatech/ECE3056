...
void compute_stats() {
    ...
    stats.aat = (stats.writebacks * DISK_PAGE_WRITE_TIME) + (stats.page_faults * DISK_PAGE_READ_TIME);
    stats.aat =  stats.aat / (double) stats.accesses;
    stats.aat = stats.aat + MEMORY_READ_TIME;
}
