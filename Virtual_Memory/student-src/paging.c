...
void system_init(void) {
    ...
    // use memset insted of for loop (piazza)
    ...
    // set frame_table pointer as protect // never overrid it
...
void proc_cleanup(pcb_t *proc) {
    ...
    for (size_t i = 0; i < NUM_PAGES; i++) {
        ...
        // access page_table entry and check if valid
        if ((page_table + i) -> valid) {

            // physical page number (PPN) -> mapped = 0
            (frame_table + page_table[i].pfn) -> mapped = 0;
        }

        if ((page_table + i) -> swap) {
