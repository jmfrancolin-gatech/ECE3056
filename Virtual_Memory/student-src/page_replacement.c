...
pfn_t free_frame(void) {
    ...
    if (frame_table[victim_pfn].mapped == 1) {
        ...
        pte_t* entry = (pte_t*) (mem + (frame_table[victim_pfn].process) -> saved_ptbr * PAGE_SIZE) + vpn;

        if (entry -> dirty == 1) {
            
            // Write back if dirty
            void* page_adress = (void*) (mem + victim_pfn * PAGE_SIZE);
            swap_write(entry, page_adress);
            entry -> dirty = 0;
